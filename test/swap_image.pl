...
#               -f     - fix truncated jpg
#               -t NUM - delete trailers larger than NUM after JPEG EOI
#               -v     - verbose
#
# Revisions:    01/25/06 - PH Created from clean_jpg
#               01/31/07 - PH Send warnings to stderr
#               09/08/09 - PH Added -t option
#
_END_
#
# Legal:        Copyright (c) 2004-2009 Phil Harvey
...
my $newData;
open FILE, "blank.jpg" or die "error opening blank.jpg\n";
...
#        print JPG_OUT $hdr or last;
        ...
        my $pos = tell JPG_IN;
        seek JPG_IN, 0, 2 or die;
        my $trailer_size = tell(JPG_IN) - $pos;
        seek JPG_IN, $pos, 0 or die;
        my $deleted = (defined $del_trailer and $trailer_size > $del_trailer);
        if ($trailer_size and not $deleted) {
        ...
        if ($verbose and $trailer_size) {
            printf("  Trailer           :%7d bytes -- %s\n", $trailer_size,
                    $deleted ? 'Cleaned' : 'Kept');
        }
        last;
      } else {
